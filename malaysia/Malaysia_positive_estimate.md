...
Based on the assumption above, we can compare the number of confirmed positive cases versus the number of deaths. In the chart below, area shaded as more likely to be under-reporting is based on countries that lie below the linear regression line when fitting data points for countries that have so far recorded less than 10,000 deaths. Based on this method, we have reasons to believe that the number of total cases reported for Malaysia so far can be used to represent the actual situation on the ground.
